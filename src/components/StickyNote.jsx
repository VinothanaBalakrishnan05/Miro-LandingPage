...
export default function StickyNote({ note, progress, mouse }) {
  ...
  const state = useRef({ opacity:0, y:80, scale:0.85, wobbleR:0 });
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const exitP  = mapClamped(p, 0.90, 0.97, 1, 0);  /* was 0.82-0.92 */
      ...
      if (ref.current && mouse.current.x > 0) {
        ...
        const dist = Math.sqrt(dx * dx + (mouse.current.y - cy) ** 2);
  ...
  return (
    <div ref={ref} className="sticky-note"
      style={{ background: note.color, boxShadow: 'var(--shadow-md)', opacity: 0 }}>
