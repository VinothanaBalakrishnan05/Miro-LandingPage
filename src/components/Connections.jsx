...
export default function Connections({ progress }) {
  ...
  const paths = useMemo(
    () => CONNECTIONS_DATA.map((c) => {
      ...
      return { ...c, d: `M ${from.x} ${from.y} Q ${mx} ${my} ${to.x} ${to.y}`, toCenter: to };
    }),
    [getCenter]
  );
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      pathRefs.current.forEach((el, i) => {
        if (!el) return;
        const conn = paths[i];
        const drawP = mapClamped(p, conn.enter, conn.enter + 0.06, 0, 1);
        const len = el.getTotalLength ? el.getTotalLength() : 300;
        el.style.strokeDasharray = len;
        el.style.strokeDashoffset = len * (1 - easeInOut(drawP));
        el.style.opacity = drawP > 0 ? mapClamped(p, 0.90, 0.97, 1, 0) : 0;  /* was 0.82-0.92 */
      });

      dotRefs.current.forEach((el, i) => {
        if (!el) return;
        const conn = paths[i];
        const showP = mapClamped(p, conn.enter + 0.04, conn.enter + 0.06, 0, 1);
        el.style.opacity = showP * mapClamped(p, 0.90, 0.97, 1, 0);
      ...
      {paths.map((path, i) => (
        <g key={i}>
          <path ref={(el) => (pathRefs.current[i] = el)}
            d={path.d} className="connection-path" vectorEffect="non-scaling-stroke" />
          <circle ref={(el) => (dotRefs.current[i] = el)}
            cx={path.toCenter.x} cy={path.toCenter.y} r="0" className="connection-dot" />
        </g>
      ))}
    </svg>
  );
